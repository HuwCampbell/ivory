...
object Definition {
  ...
  def virtual(featureId: FeatureId, source: FeatureId, expression: Expression, filter: Option[Filter], window: Option[Window]): Definition =
    Virtual(featureId, VirtualDefinition(source, expression, filter, window))
}
...
case class VirtualDefinition(source: FeatureId, expression: Expression, filter: Option[Filter], window: Option[Window]) {
  def toDefinition(featureId: FeatureId): Definition =
