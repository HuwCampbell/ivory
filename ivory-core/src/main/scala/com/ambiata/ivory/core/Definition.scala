...
object Definition {
  ...
  def virtual(featureId: FeatureId, source: FeatureId, expression: Expression, window: Option[Window]): Definition =
    Virtual(featureId, VirtualDefinition(source, expression, window))
}
...
case class VirtualDefinition(source: FeatureId, expression: Expression, window: Option[Window]) {
  def toDefinition(featureId: FeatureId): Definition =
