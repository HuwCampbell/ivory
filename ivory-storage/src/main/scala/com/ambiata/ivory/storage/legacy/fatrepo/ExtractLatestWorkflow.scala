 ...
 *    - Run snapshot app with discovered feature store
...
object ExtractLatestWorkflow {
  ...
  def onStore(repo: Repository, extractor: Extractor, date: Date, incremental: Boolean): ResultTIO[(FeatureStoreId, SnapshotId)] = {
    for {
      featureStoreId <- latestFeatureStore(repo)
      incr    <- if(incremental) SnapshotMeta.latest(repo, date) else ResultT.ok[IO, Option[(SnapshotId, SnapshotMeta)]](None)
      snap    <- decideSnapshot(repo, date, featureStoreId, incr)
      (skip, snapId) = snap
      output = repo.toReference(Repository.snapshot(snapId))
      _       <- if(skip) {
                  logger.info(s"Not running snapshot as already have a snapshot for '${date.hyphenated}' and '${featureStoreId}'")
                  ResultT.ok[IO, Unit](())
                } else {
                  logger.info(s"""
                                 ...
                                 |
                                 """.stripMargin)
                  extractor(repo, featureStoreId, date, output, incr)
                }
    } yield (featureStoreId, snapId)
  }

  def decideSnapshot(repo: Repository, date: Date, featureStoreId: FeatureStoreId, incr: Option[(SnapshotId, SnapshotMeta)]): ResultTIO[(Boolean, SnapshotId)] =
  incr.collect({ case (id, sm) if sm.date <= date && sm.featureStoreId == featureStoreId => for {
    featureStore <- Metadata.featureStoreFromIvory(repo, featureStoreId)
    partitions <- FeatureStoreGlob.between(repo, featureStore, sm.date, date).map(_.globs.flatMap(_.value.partitions))
    filtered = partitions.filter(_.date.isAfter(sm.date)) // TODO this should probably be in FeatureStoreGlob.between, but not sure what else it will affect
  ...
  def latestFeatureStore(repo: Repository): ResultTIO[FeatureStoreId] = for {
    _         <- ResultT.ok[IO, Unit](logger.info(s"Finding latest feature store in the '${repo.root.path}' repository."))
    latestOpt <- Metadata.latestFeatureStoreId(repo)
