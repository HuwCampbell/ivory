 ...
 */
class FactsReducer extends Reducer[BytesWritable, BytesWritable, NullWritable, BytesWritable] {
  ...
  override def cleanup(context: Reducer[BytesWritable, BytesWritable, NullWritable, BytesWritable]#Context): Unit =
    out.close()

  override def reduce(key: BytesWritable, iter: java.lang.Iterable[BytesWritable], context: Reducer[BytesWritable, BytesWritable, NullWritable, BytesWritable]#Context): Unit = {
    val featureId = FactsetWritable.getFeatureId(key)
    val date = FactsetWritable.getDate(key)
    val path = ReducerLookups.factsetPartitionForInt(lookup, featureId, date)
