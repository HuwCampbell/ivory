...
"""
  ...
  def e3 = prop((nsInc: Set[FeatureNamespace], nsExc: Set[FeatureNamespace], fsInc: FactsetIdList, fsExc: FactsetIdList) =>
    !nsInc.exists(nsExc.contains) ==> (Temporary.using { dir =>
      val sc = ScoobiConfiguration()
      val repo = Repository.fromHdfsPath(dir, sc)
      ...
      } yield sizes).run(sc.configuration).map(_.toSet)
    } must beOkValue(nsInc.map(ns => ns.namespace -> (fsInc.ids.length * 4).bytes)))).set(maxSize = 5, minTestsOk = 5)
