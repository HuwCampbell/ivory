...
class EavtTextImporterSpec extends Specification with ThrownExpectations with FileMatchers { def is = s2"""
  ...
  def textData = setup { setup: Setup =>
    import setup._
  ...
  def thriftData = setup { setup: Setup =>
    import setup._
  ...
  def errorMessage = setup { setup: Setup =>
    import setup._
...
class Setup(directory: FilePath) {
  ...
  lazy val namespaced = (directory </> "input" </> "ns1").path
  ...
  def save(path: String, raw: List[String]) = 
    repository.toStore.linesUtf8.write(FilePath(path) </> "part", raw)
}
