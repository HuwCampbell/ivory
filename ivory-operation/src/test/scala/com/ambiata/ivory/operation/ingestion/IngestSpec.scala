...
class IngestSpec extends Specification with SampleFacts with ScalaCheck { def is = sequential ^ section("mr") ^ section("aws") ^ s2"""
 ...
 Facts can be ingested from thrift format                                                 $thrift
    ...
    withRepository(Hdfs) { repository: Repository =>
      withCluster { cluster: Cluster =>
        withIvoryLocationDir(tt) { location =>
          ...
          Ingest.ingestFacts(repository, cluster, location, None, None, 100.mb, TextFormat).run.run(IvoryRead.create)
        }
      }
    } must beOk
  }).set(minTestsOk = 5)
    ...
    withRepository(Hdfs) { repository: Repository =>
      withCluster { cluster: Cluster =>
        withIvoryLocationDir(tt) { location =>
          ...
          Ingest.ingestFacts(repository, cluster, location, Some(Name("ns1")), None, 100.mb, TextFormat).run.run(IvoryRead.create)
        }
      }
    } must beOk
  }).set(minTestsOk = 5)
