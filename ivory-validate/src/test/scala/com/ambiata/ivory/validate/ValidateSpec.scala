...
class ValidateSpec extends Specification with ThrownExpectations with FileMatchers { def is = s2"""
  ...
  def e1 = {
    ...
    dictionaryToIvory(repo, dict, "fact1").run(sc).run.unsafePerformIO().toEither must beRight
  ...
  def e2 = {
    ...
    dictionaryToIvory(repo, dict, "dict1").run(sc).run.unsafePerformIO().toEither must beRight
